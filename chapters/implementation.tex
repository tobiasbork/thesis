...
\end{align}
Somit können wir die Berechnung in Schritt 4 von Algorithmus \ref{spca_algorithm} durch (\ref{spca_p_greater_n_solution}) ersetzen. Für den hochdimensionalen Fall kann durch das Wegfallen der Berechnung von $k$ Elastic Net Problemen ein effizientes Verfahren gewährleistet werden. Im Folgenden werden wir den modifizierten Algorithmus für $p \gg n$ mit \ref{spca_algorithm}$^*$ bezeichnen.

Bei dieser Ersetzung ist aber besondere Vorsicht zu genießen. Wir möchten kurz erklären, was dieser Spezialfall in Anwendung bewirkt. Durch die Ersetzung mit (\ref{spca_p_greater_n_solution}) wird im Wesentlichen der soft-thresholding-Operator auf die klassischen Hauptachsen angewendet. Alle Einträge in den Hauptachsen, welche im Betrag kleiner als $\frac{\lambda_{1,j}}{2}$ sind, werden dadurch auf Null gesetzt. Mit $\lambda_{1,j}$ kontrollieren wir also weiterhin den Grad der Dünnbesetzung. Allerdings werden Abhängigkeiten zwischen Variablen dabei völlig außer Acht gelassen und die Variablen als unabhängig angenommen. Auch wenn dies unzulässig scheinen mag, findet man ein derartiges Vorgehen auch in anderen Verfahren wieder \cite{tibshirani_diagnosis}. Eine Rechtfertigung dieses Ansatzes beruht allein auf guten empirischen Ergebnissen. Daher muss im Anwendungsfall entschieden werden, ob eine Verwendung von Algorithmus \ref{spca_algorithm}$^*$ sinnvoll ist.
