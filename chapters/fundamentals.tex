...
Bei der Verwendung eines solchen Modells nehmen wir an, dass eine Linearkombination der abhängigen Variablen eine geeignete Approximation ist.

Typischerweise verfügen wir über eine Menge von Trainingsdaten $(x_1, \ldots, x_n)$. Jedes $x_i = (x_{i1}, \ldots, x_{ip})^T$ stellt eine Beobachtung dar, die wir für die Schätzung der Parameter $\beta_j$ benutzen. Die bekannteste Methode für diesen Zweck ist sicherlich die \textit{Methode der kleinsten Quadraten} (Englisch: \textit{Least Squares}), in welcher wir $\beta = (\beta_0, \ldots, \beta_p)^T$ wählen, so dass die Summe der Residuenquadrate (Englisch: \textit{Residual Sum of Squares}, kurz RSS) minimiert wird. Wir definieren
\begin{align}
\label{RSS}
...
\begin{align}
\hat{\beta} = \min_{\beta} \text{RSS}(\beta)
\end{align}

wobei $\mat X \in \mathbb{R}^{n \times (p+1)}$ die Matrix der $x_i$ mit einer 1 an erster Stelle ist und $y = (y_1, \ldots, y_n)^T$ der Ausgabevektor ist. Eine Visualisierung der Methode befindet sich in Abbildung REF. Vielleicht Bild zur Regression in 2 bzw. 3D?

Es ist leicht zu zeigen, dass wir eine eindeutige Lösung (wenn X vollen Rang hat)

Wir möchten erwähnen, dass wir bei der Verwendung dieser Methode keine Aussage über die Gültigkeit des Modells treffen, sondern lediglich die beste lineare Approximation finden.


Um ein gewünschtes Verhalten der Regression zu gewährleisten, bestehen verschiedene Möglichkeiten der Regularisierung. Dazu versehen wir den Regressionsterm mit sog. \textit{Straftermen}.
...
Im folgendem Abschnitt werden wir sehen, dass die Wahl der Norm verschiedene Effekte auf die Regressionskoeffizienten hat.
