...
\begin{align}
\label{least_squares}
\hat{\beta} = \argmin_{\beta} \text{RSS}(\beta)
\end{align}
...
\begin{align}
\hat{\beta} = (\mat X^T\mat X)^{-1}\mat X^T y
\end{align}
besitzt. Die Zielgröße ergibt sich dann durch
\begin{align}
\hat{y} = \mat X \hat{\beta} = \mat X (\mat X^T\mat X)^{-1}\mat X^T y
\end{align}
...
Kurze Erklärung der Idee, warum man das anwenden möchte. Sagen, dass es verschiedene Methoden gibt, um ein geeignetes Subset zu selektieren.
Dann Probleme nennen.

\textbf{Ridge Regression}
Motivieren, warum Ridge Regression. Bias-Variance-Tradeoff
Mathematische Formulierung
\begin{align}
\hat{\beta}^{\text{ridge}} = \argmin_{\beta} \norm{y - \mat X \beta}_2^2 + \lambda \norm{\beta}_{2}^{2}
\end{align}
Mathematische Lösung des Problems
Plot wie Koeffizienten gegen 0


\textbf{LASSO}
Das LASSO ist eine 
In der Signalanalyse ist dieses Problem unter dem Namen \textit{Basis Pursuit} \cite{chen} bekannt.
Motivieren, warum LASSO. Warum möchte man viele 0 entries?
Mathematische Formulierung
\begin{align}
\hat{\beta}^{\text{lasso}} = \argmin_{\beta} \norm{y - \mat X \beta}_2^2 + \lambda \norm{\beta}_{1}
\end{align}
...
Plot mit den verschiedenen Normen im $R^2$
Intuition warum $L_1$ Norm sparsity produced
Warum nicht $p = 0.5$ als Norm?
