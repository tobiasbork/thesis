...
\hat{\beta}^{\text{en}} = \argmin_{\beta} \frac{1}{2n} \norm{y - \mat X \beta}_2^2 + \lambda \alpha \norm{\beta}_{1} + \frac{1}{2}\lambda (1-\alpha) \norm{\beta}_{2}^2
\end{align}
wobei $n$ die Anzahl an Beobachtungen im Datensatz und $\alpha \in [0,1]$ das Verhältnis zwischen $\ell_1$- und $\ell_2$-Norm ist. Mit $\alpha = 0$ reduziert sich (\ref{elastic_net_python}) auf Ridge und für $\alpha = 1$ auf die Lasso Regression. Somit können wir durch $\alpha$ das Verhältnis der beiden Regressionen kontrollieren und durch $\lambda$ die Stärke der Bestrafung. Setzt man 
\begin{align}
